...
const defaultOptions: Partial<HassWsOptions> = {
  ...
  messageParser: (incomingMessage: { data: string }) => JSON.parse(incomingMessage.data),
...
const command = async (commandArgs: HassCommandArgs, client: HassClient): Promise<any> => {
...
const connectAndAuthorize = async (client: HassClient, resolveWith: HassApi): Promise<HassApi> => {
  return new Promise((resolve, reject) => {
    ...
    client.ws.onclose = (event: CloseEvent) => client.emitter.emit('ws_close', event)
    ...
    client.emitter.on('auth_invalid', (msg: { message: string }) => reject(new Error(msg.message)))
    client.emitter.on('auth_required', () => {
      ...
      if (!client.options.token) {
        reject(
          new Error('Homeassistant requires authentication, but token not provided in options'),
        )
      }
